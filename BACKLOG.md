...
- **Code-mode gateway spike (Executor)** (added 2026-09-20) — "code mode" MCP (Cloudflare Code Mode, Anthropic's code-execution-with-MCP, gateways like executor.sh) gives the model one `execute` tool and exposes MCP tools as a generated TypeScript API; scripts run in a sandbox, so intermediate results (search → notes per WO → filter/group) never pass through the LLM's context. Executor wraps existing MCP servers, so this server needs no changes to be tried. Spike: register `sc-workorders` behind Executor (local, stdio), run one composed script (search open HVAC WOs → notes for each → summarize) against the same question via direct tool calls, and compare with `SC_METRICS_FILE` (tokens, ms, apiCalls). Record whether the ~20 req/min limit bites and whether Executor surfaces `structuredContent`. Tool-definition bloat is not the win here (8 small tools); composition is. Decision input for the three code-mode items below and for `get_work_order_context`.
- **`outputSchema` on all 8 tools** (added 2026-09-20) — none declares one today (`structuredContent` is emitted but untyped). Code-mode gateways generate typed TS from tool schemas; without output schemas the script author sees `unknown` and guesses field names. Zod schemas co-located with each `toCompact*` mapper (same rule as the `$select` constants); add a unit assertion that each mapper's output parses against its schema. Small effort, useful to non-code-mode clients too (validation).
- **`get_work_order_context`** (composite tool bundling a work order + notes + assets + invoice in one call) — added 2026-09-20: code mode makes hand-built composites less necessary, so keep this only if the spike above (or metrics) shows plain, non-code-mode clients need it; otherwise demote. Every underlying piece now exists and is confirmed working (`get_work_order`, `get_work_order_notes`, `get_work_order_assets`, invoice-on-work-order). Promoted from Later now that its dependencies shipped in v0.4.0 — no more sequencing blocker, just the build itself. The truncation question is already resolved (`get_work_order_assets`'s `{count, totalCount, truncated}` shape), so this just needs to compose the existing calls.
