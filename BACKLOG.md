...
Ordered. 1 and 2 are the gaps for anyone installing this from GitHub or measuring it; 3 onward are capability.
...
- **`count_work_orders` with group-by status or trade** (added 2026-09-18, unverified) — answer "how many HVAC WOs are open?" in one call instead of one call per status. **`$apply=groupby` ruled out 2026-09-18:** HTTP 400 ("'with' expected at position 42"), so aggregation is not supported here. Route left: one `$count` (via count-only mode) per status value composed server-side, with the status list discovered rather than hardcoded (`Status/Primary` isn't a closed enum). Depends on count-only mode. Location rollup (Later) can reuse the same spike.
