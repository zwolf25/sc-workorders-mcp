...
**Re-prioritized 2026-09-24** on the same three axes, after `get_work_order_context` shipped in v0.7.0. Ordered by value against triage questions per unit of effort: item 1 is the most direct triage gain and mostly verified, 2 is small with no unknowns, 3 is the biggest unknown and gates two Later items, 4 is a one-liner.

- **More `search_work_orders` filters** (added 2026-09-18, unverified; **top of the list as of 2026-09-24**) — `contains(Description,'x')` text search (`contains(Description,...)` is accepted, HTTP 200, but the one probe returned 0 matches, so match behavior is still unconfirmed; retry with a term known to appear); `priority`, `tradeId` and `categoryId` filters (the IDs are already returned but can't be filtered on, and `tradeId` would skip the exact-string trade lookup); `locationName`, resolved internally (today it takes `search_locations` then `search_work_orders`). Any new filter goes through the `buildFilter` whitelist. Order: ID filters first (`tradeId`/`priority`/`categoryId`, the IDs are already returned and are plain `eq` clauses), then re-probe `Description` with a known term, then `locationName`.
- **`outputSchema` on all 9 tools** (added 2026-09-20) — none declares one today (`structuredContent` is emitted but untyped). Code-mode gateways generate typed TS from tool schemas; without output schemas the script author sees `unknown` and guesses field names. Zod schemas co-located with each `toCompact*` mapper (same rule as the `$select` constants); add a unit assertion that each mapper's output parses against its schema. Small effort, no unknowns, useful to non-code-mode clients too (validation).
- **Code-mode gateway spike (Executor)** (added 2026-09-20) — "code mode" MCP (Cloudflare Code Mode, Anthropic's code-execution-with-MCP, gateways like executor.sh) gives the model one `execute` tool and exposes MCP tools as a generated TypeScript API; scripts run in a sandbox, so intermediate results (search → notes per WO → filter/group) never pass through the LLM's context. Executor wraps existing MCP servers, so this server needs no changes to be tried. Spike: register `sc-workorders` behind Executor (local, stdio), run one composed script (search open HVAC WOs → notes for each → summarize) against the same question via direct tool calls, and compare with `SC_METRICS_FILE` (tokens, ms, apiCalls). Record whether the ~20 req/min limit bites and whether Executor surfaces `structuredContent`. Tool-definition bloat is not the win here (9 small tools); composition is. Decision input for the two code-mode items in Later. Also the natural place to run the pending `SC_METRICS_FILE` comparison of `get_work_order_context` (2 requests) against 3 separate calls.
- **Environment guard** (added 2026-09-18, moved up 2026-09-24: one startup log line, no design questions) — `SC_API_BASE_URL` swaps between sandbox and prod silently. A startup log line naming the environment (host only, never credentials) would prevent "which env am I querying?" confusion.
...
- **Rate-limit-aware errors for loop callers** (added 2026-09-20, gated on the code-mode spike) — scripts in a code-mode sandbox loop fast and will hit the ~20 req/min limit; blocked attempts extend the window. Make the 429 machine-readable (e.g. `retryAfterMs` in the structured error) so a script can sleep instead of hammering; optionally a small in-process throttle (~18/min). **Touches the "retry/backoff engineering" out-of-scope item — needs Zac's explicit go-ahead before building.** Only worth doing if the spike shows real 429s.
- **README/ARCHITECTURE section "Using with code-mode gateways"** (added 2026-09-20) — how to register with Executor, the rate-limit caveat, and that credentials stay in the local env. Write after the spike so it records observed behavior.
...
### v0.7.0 (2026-09-23)
- `get_work_order_context` — work order + provider + invoice + assets in one request, plus notes: 2 API requests instead of 3. A throttled notes request returns the rest with `notes: null, notesTruncated: true`. Live-verified (`npm test`). Still to measure with `SC_METRICS_FILE`: bundle vs 3 separate calls.
...
- **Built-in `execute`/sandbox tool in this server** (code mode inside the server) — rejected 2026-09-20. Arbitrary code execution undermines the whitelist-only safety boundary (`buildFilter`/`buildOrderBy`), adds sandbox/monitoring ops burden (Anthropic's own stated caveat), and most sandboxes need remote/HTTP transport, which is out of scope. Gateways like Executor already provide that layer around this server. Revisit only if a gateway can't be used.
- **Batch inputs (`workOrderIds[]`) on notes/activities/assets** — rejected 2026-09-20. Each is a separate sub-resource GET, so batching saves no API calls; the ~20 req/min rate limit, not token count, is the bottleneck. Code mode already removes the token cost of looping.
